...
from drive.Stanford.CS231n_master.assignment2.cs231n.layers import *
from drive.Stanford.CS231n_master.assignment2.cs231n.layer_utils import *
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        grads['W2'] = dw2 + self.reg * self.params['W2'] # 為什麼不是用減
        grads['b2'] = db2
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        ############################################################################
        for i in range(len(hidden_dims)):  # 把w0,b0 w1 b1 初始完成
            layer = hidden_dims[i]
            self.params['W'+str(i+1)] = np.random.normal(0, weight_scale, size=(input_dim, layer))
            self.params['b'+str(i+1)] = np.zeros(layer)
            if self.use_batchnorm:
                self.params['gamma'+str(i+1)] = np.ones(layer) # 為何一個1
                self.params['beta'+str(i+1)] = np.zeros(layer) #  全部0
            input_dim = layer # layer 裡面已經把 該層的神經元取出 
        ...
        if self.use_dropout:
            ...
            #seed 預設是none     如果不是none 有給予值 才會進來 將值給予 self.dropout_paramm['seed']
            if seed is not None:
    ...
    def loss(self, X, y=None):
        ...
        #if y is None then test      else train
