...
def affine_backward(dout, cache):
    ...
    Inputs:
    - dout: Upstream derivative, of shape (N, M) M是啥
    - cache: Tuple of:
    ...
    ###########################################################################
    # dx.shape (10,6)
    dx = dout.dot(w.T).reshape(x.shape) # 為何要.T       dout = (10,5)   w.T = (5, 6 ) 
   
    # x.reshape(x.shape[0], -1) = (10,6)   轉至後變(6,10) dout = 10 5 
    # dw.shape (6,5)
    dw = x.reshape(x.shape[0], -1).T.dot(dout) # 要將所有內容 向量化
    db = np.sum(dout, axis=0) # axis = 0  從row 去做相加  每個row相同的索引為製作相加
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dw, db
...
def relu_forward(x):
    ...
    ###########################################################################
    out = np.maximum(0, x) # 把負號都用0取代
    #print("relu_forward_out :",out)
    print("relu_forward_out.shape :",out.shape) #(100, 100)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = x
...
def svm_loss(x, y):
    ...
    # correct_class_scores[:, np.newaxis] + 1.0  將correct_class_scores 增加一個維度(為什麼要增加一個維度?) 
    margins = np.maximum(0, x - correct_class_scores[:, np.newaxis] + 1.0)
    print("correct_class_scores[:, np.newaxis].shape :",correct_class_scores[:, np.newaxis].shape)
...
def softmax_loss(x, y):
    ...
    - dx: Gradient of the loss with respect to x
    """
    shifted_logits = x - np.max(x, axis=1, keepdims=True) # 先將X最大的值取出  然後用 x 所有col 的值 減掉 x 取出的最大值  ...此步驟可做可不做
    Z = np.sum(np.exp(shifted_logits), axis=1, keepdims=True)# 
    log_probs = shifted_logits - np.log(Z)# 因為這邊需要將 矩陣內 所有的值 變成機率值  log
    probs = np.exp(log_probs) # 這邊將log 轉回指數  因為probs裡面是之後要計算梯度用
    N = x.shape[0]
    loss = -np.sum(log_probs[np.arange(N), y]) / N # 因為-log 所以-np.sum   loss 是要計算 在正確類別得分的損失是多少 除上N 算出平均損失函數 
    dx = probs.copy()   
    dx[np.arange(N), y] -= 1 #???
    dx /= N
    return loss, dx
